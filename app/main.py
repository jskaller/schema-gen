...
app = FastAPI(title="Schema Gen", version="1.6.0")
...
async def _process_single(
    ...
    # Normalize the primary node
    inputs = {"topic": topic, "subject": subject, "address": address, "phone": phone}
    primary_node = normalize_jsonld(base_jsonld, primary_type, inputs)

    # Assemble @graph with secondary stubs if needed
    if secondary_types:
    ...
    valid, errors = validate_against_schema(final_jsonld["@graph"][0] if "@graph" in final_jsonld else final_jsonld, schema_json)
    overall, details = score_jsonld(final_jsonld, effective_required, effective_recommended)
    tips = advise(final_jsonld, effective_required, effective_recommended)

    # (Optional refine step could be adapted to graph; keep as-is for now)
    iterations = 0
