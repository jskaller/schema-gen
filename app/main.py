...
app = FastAPI(title="Schema Gen", version="1.10.0")
...
async def _startup():
    await init_db()
    print("[startup] app v1.10.0", file=sys.stderr)
...
async def _quick_competitor_score(url: str, primary_type: str, session: AsyncSession, provider_name: str, provider_model: str | None):
    ...
    provider = get_provider(provider_name, model=provider_model)
    try:
        base_jsonld = await asyncio.wait_for(provider.generate_jsonld(payload), timeout=GEN_TIMEOUT)
    except Exception:
        base_jsonld = {}
    try:
        primary_node = normalize_jsonld(base_jsonld or {}, primary_type, {"url": url})
    except Exception:
        primary_node = {"@context": "https://schema.org", "@type": primary_type, "url": url}
    try:
        final_jsonld = assemble_graph(primary_node, [], url or "", {"url": url})
    except Exception:
    ...
    node_with_markers = dict(root_node or {})
    node_with_markers["__valid__"] = valid
    node_with_markers["__errors__"] = errors
    try:
        overall, _ = score_jsonld(node_with_markers, defs["required"], defs["recommended"])
    except Exception:
        overall = 0
    return {"url": url, "overall": overall, "valid": bool(valid)}
...
async def _process_single(url: str, topic, subject, audience, address, phone, compare_existing, competitor1, competitor2, label, session: AsyncSession, job_id: str | None = None):
    ...
    try:
        ...
        node_with_markers["__valid__"] = valid
        node_with_markers["__errors__"] = errors
        overall, details = score_jsonld(node_with_markers, effective_required, effective_recommended)
    except Exception:
    ...
    # Competitor scoring (if provided)
    comparisons = []
    comp_notes = []
    if (competitor1 and competitor1.strip()) or (competitor2 and competitor2.strip()):
        await hb(97, "Scoring competitors")
        prov = s.provider or "dummy"
        prov_model = s.provider_model or None
        if competitor1 and competitor1.strip():
            try:
                c1 = await _quick_competitor_score(competitor1.strip(), primary_type, session, prov, prov_model)
                comparisons.append(c1)
            except Exception as e:
                comp_notes.append(f"Competitor1 failed: {e}")
        if competitor2 and competitor2.strip():
            try:
                c2 = await _quick_competitor_score(competitor2.strip(), primary_type, session, prov, prov_model)
                comparisons.append(c2)
            except Exception as e:
                comp_notes.append(f"Competitor2 failed: {e}")
