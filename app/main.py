...
app = FastAPI(title="Schema Gen", version="1.5.3")
templates = Jinja2Templates(directory="app/web/templates")

# ---- Debug helpers -----------------------------------------------------------
def log_routes(when: str):
    try:
        paths = []
        ...
        print(f"[ROUTES {when}] failed to list routes: {e}", file=sys.stderr)
...
async def submit(
    ...
    # Debug trace to stderr to confirm the handler is invoked
    print("[/submit] handler invoked", file=sys.stderr)
    if not url:
...
async def admin_test(request: Request, session: AsyncSession = Depends(get_session)):
    ...
    models = await list_ollama_models()
    return templates.TemplateResponse("admin.html", {"request": request, "settings": s, "test_result": result, "ollama_models": models, "page_types": AVAILABLE_PAGE_TYPES})
